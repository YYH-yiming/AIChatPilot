...
public class KnowledgeBaseSelector {
    ...
    public SelectionResult ask(String query, Long requestedKbId, Integer topK) {
        if (requestedKbId != null && requestedKbId > 0) {
            KnowledgeAskClientResponse response = knowledgeServiceClient.ask(requestedKbId, query, topK);
            return new SelectionResult(requestedKbId, response, "request-kbid");
        }
        ...
        if (selectedByLlm.isPresent()) {
            ...
            KnowledgeAskClientResponse response = knowledgeServiceClient.ask(selected.getId(), query, topK);
        ...
        KnowledgeBaseClientResponse fallback = knowledgeBases.get(0);
        KnowledgeAskClientResponse response = knowledgeServiceClient.ask(fallback.getId(), query, topK);
    ...
    private Optional<KnowledgeBaseClientResponse> selectByLlm(String query,
                                                              List<KnowledgeBaseClientResponse> knowledgeBases) {
        try {
            String answer = chatLlmService.chat(buildSystemPrompt(), buildUserPrompt(query, knowledgeBases));
            Decision decision = parseDecision(answer, knowledgeBases);
            ...
            log.warn("Chat知识库LLM选库失败，将退回全库检索: {}", ex.getMessage());
            return Optional.empty();
        }
    }
    ...
    private SearchCandidate toSearchCandidate(KnowledgeBaseClientResponse base, String query, Integer topK) {
        try {
            ...
            log.warn("Chat全库检索候选失败: kbId={}, message={}", base.getId(), ex.getMessage());
            return null;
        }
    }
    ...
    private String buildUserPrompt(String query, List<KnowledgeBaseClientResponse> knowledgeBases) {
