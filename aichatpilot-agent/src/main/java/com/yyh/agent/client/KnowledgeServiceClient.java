...
public class KnowledgeServiceClient {
     ...
     * 用 JDK HttpClient（不引新依赖），透传 X-User-Id/X-Tenant-Id（直连 knowledge，无网关翻译 JWT）。镜像 chat 侧 askStream。
     */
    public StreamAskResult askStream(Long kbId, String query, Integer topK,
                                     Consumer<KnowledgeAskClientResponse> onMeta,
                                     Consumer<String> onToken) {
        if (kbId == null || kbId <= 0) {
            throw new BusinessException(ResultCode.PARAM_ERROR, "知识库流式必须提供 kbId");
        }
        String url = agentProperties.getKnowledge().getServiceUrl()
                + "/api/knowledge/bases/" + kbId + "/ask/stream";
        String body;
        try {
            body = objectMapper.writeValueAsString(buildAskRequest(query, topK));
        } catch (Exception ex) {
    ...
    /** 流式问答汇总：完整答案、元信息（references/grounded/kbId/refCount）、总 token。 */
